    ...
    return (
        <>
            <div className="flex-1 flexCenter dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 px-4 rounded-md py-3">
                <Image src={images.search} alt={search} style={{ objectFit: 'contain' }} width={20} height={20} className={theme === 'light' && 'filter invert'}/>
                <input type="text"
            ...
            <div onClick={() => setToggle((prevState) => !prevState)} className="relative flexBetween ml-4 sm:sm-0 sm:mt-2 min-w-190 cursor-pointer dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 px-4 rounded-md">
                <p className="font-poppins dark:text-white text-nft-black-1 font-normal text-xs">Recently Listed</p>
                <Image src={images.arrow} alt='arrow' style={{ objectFit: 'contain' }} width={15} height={15} className={theme === 'light' && 'filter invert'}/>
                {toggle && (
                    <div className="absolute top-full left-0 right-0 w-full mt-3 z-10 dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md">
                        {['Recently added', 'Price (low to high)','Price (high to low)'].map((item) => (
                            <p className="font-poppins dark:text-white text-nft-black-1 font-normal text-xs my-3 cursor-pointer" onClick={() => setActiveSelect(item)} key={item}>{item}</p>
                        ))}
                    </div>
                )}
            </div>
